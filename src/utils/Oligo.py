...
class Oligo:
    ...
    def cal_oligo_mass(self, elements, nucls):
        from src.utils.Nucleotide import cal_mass_by_sequence
    ...
    def cal_fragments_mass(self, elements: dict, nucleotides, mods, max_charge=-5):
        ...
        #
        mods_index = [[0.0, 0.0, 0.0]] * len(self.sequence)

        for info in self.sequence_location[0]['mod_infos']:  # 多个母离子同一个self的时候，mods位置一样才合并
            mods_index[info['pos']] += [mods[info['mod_symbol']].base_delta_mass,
                                        mods[info['mod_symbol']].sugar_delta_mass,
                                        mods[info['mod_symbol']].phosphate_delta_mass]
        ...
        #     if i in mods_poses:
        #         icon_mass += [mods[mods_index[i]].base_mass, nucleotides[self.sequence[i]].sugar_mass,
        #         #                  nucleotides[self.sequence[i]].phosphate_mass]
        #
        ...
        if max_charge > 0:
            print("Sorry, current version don't support positive mod")
            return
        ...
        for i in range(len(self.sequence)):
            ...
            # a-B
            cur_mass_5 += nucleotides[B].sugar_mass + mods_index[i][0] - elements["H"][0][0] * 3
            flag = ord('a')

            for j in range(1, max_charge + 1):
                label1 = "a_{}-{}^{}".format(label_id, B, j)
                tmp_mass = (cur_mass_5 - elements["A"][0][0] * (j - 1)) / j
                masses.append((label1, tmp_mass))

            # a
            cur_mass_5 += nucleotides[B].base_mass + mods_index[i][1] + elements["H"][0][0]

            for j in range(1, max_charge + 1):
                label1 = "{}_{}^{}".format(chr(flag), label_id, j)
                masses.append((label1, (cur_mass_5 - elements["A"][0][0] * (j - 1)) / j))

            # w
            cur_mass_3 = whole_mass - cur_mass_5 - elements["A"][0][0] * 2
            # - elements["e"][0][0]

            for j in range(1, max_charge + 1):
                label2 = "{}_{}^{}".format(chr(flag + 22), seq_len - label_id, j)
                masses.append((label2, (cur_mass_3 - elements["A"][0][0] * (j - 1)) / j))

            # b
            cur_mass_5 += cal_mass_by_formula(elements, "H(2)O(1)")

            for j in range(1, max_charge + 1):
                label1 = "{}_{}^{}".format(chr(flag + 1), label_id, j)
                masses.append((label1, (cur_mass_5 - elements["A"][0][0] * (j - 1)) / j))
            ...
            for j in range(1, max_charge + 1):
                label2 = "{}_{}^{}".format(chr(flag + 22 + 1), seq_len - label_id, j)
                masses.append((label2, (cur_mass_3 - elements["A"][0][0] * (j - 1)) / j))

            # c
            cur_mass_5 += cal_mass_by_formula(elements, "H(-1)O(2)P(1)") + mods_index[i][2]

            for j in range(1, max_charge + 1):
                label1 = "{}_{}^{}".format(chr(flag + 2), label_id, j)
                masses.append((label1, (cur_mass_5 - elements["A"][0][0] * (j - 1)) / j))
            ...
            for j in range(1, max_charge + 1):
                label2 = "{}_{}^{}".format(chr(flag + 22 + 2), seq_len - label_id, j)
                masses.append((label2, (cur_mass_3 - elements["A"][0][0] * (j - 1)) / j))
            ...
            for j in range(1, max_charge + 1):
                label1 = "{}_{}^{}".format(chr(flag + 3), label_id, j)
                masses.append((label1, (cur_mass_5 - elements["A"][0][0] * (j - 1)) / j))
            ...
            for j in range(1, max_charge + 1):
                label2 = "{}_{}^{}".format(chr(flag + 22 + 3), seq_len - label_id, j)
                masses.append((label2, (cur_mass_3 - elements["A"][0][0] * (j - 1)) / j))
