...
from src.flow.mod_digest import find_mods
from src.utils.Element import create_elements_from_file
from src.utils.Enzyme import get_used_enzymes
from src.utils.Modification import create_modifications_from_table, get_known_mod
...
def flows_continuing(test_k, test_b):
        ...
        for f in mgf_datas:
            while True:
                try:
                    ...
                    from src.flow.enzyme_digest import digest_to_oligos, shrink_oligos
    ...
    for oli in all_oligos:
        ...
        with open("tests\\results\\02_AGUC\\oligo_ions_" + oli.chemistry_5 + "_" + oli.sequence + oli.sequence
    ...
    with concurrent.futures.ThreadPoolExecutor() as executor:
        for m in mgf_datas:
            for spectrum in m:
                ...
                match_futures.append(executor.submit(match, spectrum, all_oligos, match_dict))

                with open("tests\\results\\02_AGUC\\spectrum_" + spectrum['params']['title'] + ".csv", 'w',
                          newline='') as file:
        ...
        with open("tests\\results\\02_AGUC\\best_matches.csv", 'w', newline='') as file:
            file.write("matched_score,oligo_sequence,oligo_mass,spectrum_pepmass,tda_fdr,is_decoy\n")
...
if __name__ == '__main__':
    ...
    print(flows_continuing(0.2, 4.1))
